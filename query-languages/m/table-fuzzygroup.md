...
* `IgnoreSpace`: A logical (true/false) value that allows combining of text parts in order to find groups. For example, when true, "Gra pes" is grouped with "Grapes". However, even when true, some spaces still aren't ignored (for example, "M 365" isn't grouped with "M365"). The default value is true.
