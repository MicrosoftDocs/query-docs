...
|------------------------------|-----------------------------------------------|----------------|--------------------------------------------|--------------------------------------------------|-----------------------------------------------------------|
| [`DateTime.LocalNow`](datetime-localnow.md)          | A `datetime` representing the current local time          | Dynamic&mdash;returns a new value each time it's invoked during query evaluation       | Returns local machine time                 | Returns UTC time  | Quick local timestamp without time zone context          |
| [`DateTimeZone.LocalNow`](datetimezone-localnow.md)      | A `datetimezone` value representing the current local time with time zone offset     | Dynamic&mdash;returns a new value each time it's invoked during query evaluation       | Returns local time with offset             | Returns UTC time with `+00:00` offset            | Local time with time zone awareness                      |
| [`DateTime.FixedLocalNow`](datetime-fixedlocalnow.md)     | A `datetime` value representing the local time when first invoked during query evaluation         | Fixed&mdash;returns the same value throughout a single query evaluation          | Captures local time when first called                | Captures UTC time when first called                        | Snapshot of local time without time zone                 |
| [`DateTimeZone.FixedLocalNow`](datetimezone-fixedlocalnow.md) | A `datetimezone` value representing the local time with offset when first invoked during query evaluation     | Fixed&mdash;returns the same value throughout a single query evaluation          | Captures local time with offset when first called    | Captures UTC time with `+00:00` offset when first called   | Snapshot of local time with time zone                    |
| [`DateTimeZone.UtcNow`](datetimezone-utcnow.md)        | A `datetimezone`  value representing the current UTC time     | Dynamic&mdash;returns a new value each time it's invoked during query evaluation        | Returns current UTC time                   | Returns current UTC time                         | Consistent UTC timestamp for dynamic scenarios           |
| [`DateTimeZone.FixedUtcNow`](datetimezone-fixedutcnow.md)   | A `datetimezone` value representing the UTC time when first invoked during query evaluation      | Fixed&mdash;returns the same value throughout a single query evaluation          | Captures UTC time when first called                  | Captures UTC time when first called                        | Fixed UTC timestamp for logging or auditing              |

In Power Query M, choosing between local time and UTC-based date and time functions is a critical design decision that affects the consistency, accuracy, and portability of your queries. Functions like [`DateTime.LocalNow`](datetime-localnow.md) and [`DateTime.FixedLocalNow`](datetime-fixedlocalnow.md) are useful when your logic depends on the local system time, such as filtering for records that occurred "today" or generating timestamps for user-facing reports. These functions reflect the time zone of the environment in which the query is executed, making them suitable for Power Query Desktop scenarios where the local context is well-defined.

However, in distributed or cloud-based environments like Power Query Online, these same functions return UTC time, not the actual local time of the user. This discrepancy can lead to subtle inconsistencies if your logic assumes a local time context. In contrast, [`DateTimeZone.UtcNow`](datetimezone-utcnow.md) and [`DateTimeZone.FixedUtcNow`](datetimezone-fixedutcnow.md) provide a time-zone-neutral reference point that is consistent across environments and unaffected by daylight saving time or regional settings. These UTC-based functions are the preferred choice for scenarios involving data integration, logging, auditing, or any logic that must behave identically regardless of where or when the query runs.
...
* [`DateTime.LocalNow`](datetime-localnow.md) returns the current local `datetime` each time the expression is evaluated.
* [`DateTime.FixedLocalNow`](datetime-fixedlocalnow.md) returns the local `datetime` once per query evaluation, acting as a snapshot.
* [`DateTimeZone.LocalNow`](datetimezone-localnow.md) returns the current local `datetimezone` each time the expression is evaluated.
* [`DateTimeZone.FixedLocalNow`](datetimezone-fixedlocalnow.md) returns the local `datetimezone` once per query evaluation, acting as a snapshot

To demonstrate the difference, the following example generates a table with multiple rows. Each row captures a fresh [`DateTime.LocalNow`](datetime-localnow.md) value using a delay to ensure distinct timestamps, while each captured [`DateTime.FixedLocalNow`](datetime-fixedlocalnow.md) value remains constant across all rows.
...
:::image type="content" source="media/m-local-fixed-utc-variants/localnow-versus-fixedlocalnow.png" alt-text="Screenshot of the table created by [`DateTime.LocalNow`](datetime-localnow.md) with dynamic dates and times and [`DateTime.FixedLocalNow`](datetime-fixedlocalnow.md) with fixed dates and times.":::

If you look at the output, you might notice that even though the [`DateTime.LocalNow`](datetime-localnow.md) function appears first in the code, the value returned for [`DateTime.FixedLocalNow`](datetime-fixedlocalnow.md) shows a time that occurs before the `DateTime.LocalTime` time. Even though [`DateTime.LocalNow`](datetime-localnow.md) is listed first in the table construction, the order of evaluation in Power Query M isn't guaranteed to follow the order of fields in a table. Instead, Power Query uses a lazy evaluation model. Using this model means that fields are only evaluated when needed and the engine determines the evaluation order, not the order in your code. In this case, the [`DateTime.FixedLocalNow`](datetime-fixedlocalnow.md) function is evaluated first, so the first time returned for this function occurs before the first time returned for [`DateTime.LocalNow`](datetime-localnow.md).

The following example shows how to produce similar results using [`DateTimeZone.LocalNow`](datetimezone-localnow.md) and [`DateTimeZone.FixedLocalNow`](datetimezone-fixedlocalnow.md).
...
:::image type="content" source="media/m-local-fixed-utc-variants/zone-localnow-versus-fixedlocalnow.png" alt-text="Screenshot of the table created by [`DateTimeZone.LocalNow`](datetimezone-localnow.md) with dynamic dates and times and [`DateTimeZone.FixedLocalNow`](datetimezone-fixedlocalnow.md) with fixed dates and times.":::
...
* [`DateTimeZone.UtcNow`](datetimezone-utcnow.md) returns the current UTC `datetimezone` each time the expression is evaluated.
* [`DateTimeZone.FixedUtcNow`](datetimezone-fixedutcnow.md) returns the local `datetimezone` once per query evaluation, acting as a snapshot.
...
:::image type="content" source="media/m-local-fixed-utc-variants/zone-utcnow-versus-fixedutcnow.png" alt-text="Screenshot of the table created by [`DateTimeZone.UtcNow`](datetimezone-utcnow.md) with dynamic dates and times and [`DateTimeZone.FixedUtcNow`](datetimezone-fixedutcnow.md) with fixed dates and times.":::
...
* **Be explicit about time zones**: Use the DateTimeZone functions instead of DateTime functions when time zone context matters. Use [`DateTimeZone.UtcNow`](datetimezone-utcnow.md) or [`DateTimeZone.FixedUtcNow`](datetimezone-fixedutcnow.md) for consistency across environments, especially in cloud-based solutions like Power BI service.
* **Use fixed functions for repeatable results**: Use the fixed variants (such as [`DateTimeZone.FixedUtcNow`](datetimezone-fixedutcnow.md)) when you want the timestamp to remain constant across query evaluations. This method is especially useful for logging, auditing, or capturing the time of data ingestion.
* **Avoid local functions in Power Query Online**: Functions like [`DateTime.LocalNow`](datetime-localnow.md) and [`DateTimeZone.LocalNow`](datetimezone-localnow.md) return UTC time in cloud-based solutions like Power BI service, which can lead to confusion or incorrect assumptions. If you need actual local time in the service, consider adjusting UTC manually using known offsets (although this adjustment can be brittle, for example, due to daylight savings time or regional settings).
