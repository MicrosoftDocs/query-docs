...
|**JoinAlgorithm.RightIndex**|6| In batches, uses the keys from the right table to do predicate-based queries against the left table. This algorithm is recommended when the left table is large, supports folding of [`Table.SelectRows`](table-selectrows.md), and contains few rows that are expected to match a right row.|
