...
Compatibility between a given type and a ? type can be determined using the library function `Type.Is`, which accepts an arbitrary type value as its first and a ? type value as its second argument:
