...
* `IgnoreSpace`: A logical (true/false) value that allows combining of text parts in order to find matches. For example, when true, "Gra pes" is matched with "Grapes". However, even when true, some spaces still aren't ignored (for example, "M 365" isn't matched with "M365"). The default value is true.
