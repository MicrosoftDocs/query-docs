...
<!-- Reviewers - What should the phrase "format strings" be changed to for Power Query M? "Format text" or something else? -->
...
- To define the text representation that results from a formatting operation.
<!-- Reviewers - Since this second use doesn't seem to be relevant for Power Query M, how to rewrite the first paragraph?-->
...
<!-- Reviewers - Is the following paragraph and sample relevant to Power Query M? If it is, how should it be changed? -->
...
<!-- Reviewers - I don't think this can be done in Power Query M - can't retrieve patterns using just M. Is this correct?
...
<!-- Reviewers - do we want the use the phrase "invariant culture" since not listing the culture in Power Query M appears to use "en-US" as it's default culture? Also, other places in the text calls this "unspecified culture" - are these the same thing?-->
...
The "O" or "o" standard format specifier represents a custom date and time format string using a pattern that preserves time zone information and emits a result string that complies with ISO 8601. For **DateTimeZone** values, this format specifier is designed to preserve date, time, and timezone values in text.<!-- The rest isn't relevant >>>  The formatted string can be parsed back by using the <xref:System.DateTime.Parse%28System.String%2CSystem.IFormatProvider%2CSystem.Globalization.DateTimeStyles%29?displayProperty=nameWithType> or <xref:System.DateTime.ParseExact%2A?displayProperty=nameWithType> method if the `styles` parameter is set to <xref:System.Globalization.DateTimeStyles.RoundtripKind?displayProperty=nameWithType>. -->

The "O" or "o" standard format specifier corresponds to the "yyyy'-'MM'-'dd'T'HH':'mm':'ss'.'fffffffK" custom format string for **DateTimeZone** values<!-- Reviewers - is the last part of this sentence relevant to Power Query? >>> ...and to the "yyyy'-'MM'-'dd'T'HH':'mm':'ss'.'fffffffzzz" custom format string for <xref:System.DateTimeOffset> values-->. In this text, the pairs of single quotation marks that delimit individual characters, such as the hyphens, the colons, and the letter "T", indicate that the individual character is a literal that can't be changed. The apostrophes don't appear in the output string.

The "O" or "o" standard format specifier (and the "yyyy'-'MM'-'dd'T'HH':'mm':'ss'.'fffffffK"  custom format) takes advantage of the three ways that ISO 8601 represents time zone information to preserve the **DateTimeZone** values:
...
<!-- Reviewers - The unspecified values do seem to have time Zone information in Power Query - so what to do with this third bullet?-->
...
<!--retest this one Monday on work system because final result is for central time not pacific time-->
```powerquery-m
let
...
<!-- Need to add the Power Query Desktop/PowerQuery Online note here -->
<!-- Reviewers - Is the next paragraph and sample relevant for Power Query M?
...
-->
<!-- Reviewers - Is the following paragraph relevant for Power Query M? If so, how should it be worded? -->
...
<!-- Reviewers - This code gives the correct answers, but I'm not sure it really shows what's being discussed in this section. Should it be changed and, if so, what are the changes?-->
```powerquery-m
...
<!-- Reviewers - How much of the next paragraph can be kept and how should it be rewritten (if kept)?-->
...
<!--Reviewers - Could not get this format specifier to work at all - the U format produces an Expression.Error: The output DateTimeZone format specified isn't supported. Remove this section? -->
...
<!-- the rest of this article is not relevant for Power Query M
