...
The **each** keyword is used to create simple functions. The expression `each ...` is shorthand for a function signature that takes a single parameter named `_`. So `each ...` is really just a streamlined way of saying `(_) => ...`.

The **each** keyword is useful when combined with the lookup operator, which is applied by default to an identifier named `_`.
