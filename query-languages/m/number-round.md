...
* `roundingMode`: Overrides the default tie-breaking behavior when `number` is at the midpoint between two potential rounded values (see [RoundingMode.Type](roundingmode-type.md) for possible values).
