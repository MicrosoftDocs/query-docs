...
```dax
GROUPCROSSAPPLYTABLE( <groupBy_columnName> [, < groupBy_columnName >]…, [<filterTable>]… [, <separator>, <tableExpression>] )  
```
...
|`separator`|A string literal which serves no purpose other than separating filterTable parameter with tableExpression parameter|  
|`tableExpression`|A table expression that is evaluated under filter context of filterTable parameters and returned as a part of the join in the return value|
